...
if not defined PQC_OPENSSL_PATH if exist "%ProgramFiles%\OpenSSL-Win64\bin\openssl.exe" set "PQC_OPENSSL_PATH=%ProgramFiles%\OpenSSL-Win64\bin\openssl.exe"

if defined PQC_OPENSSL_PATH (
    "%PQC_OPENSSL_PATH%" version
    if errorlevel 1 echo WARNING: PQC_OPENSSL_PATH is not executable.
    "%PQC_OPENSSL_PATH%" version | findstr /r /c:"OpenSSL 3\.[5-9]" /c:"OpenSSL [4-9]\." >nul
    ...
    if errorlevel 1 (
        ...
        openssl version
        openssl version | findstr /r /c:"OpenSSL 3\.[5-9]" /c:"OpenSSL [4-9]\." >nul
        if errorlevel 1 echo WARNING: OpenSSL 3.5+ PQC groups unavailable.
    )
)
